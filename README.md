...
ファイル名の先頭に`_`をつけた場合はモジュールファイルとして解釈されるため、直接HTMLファイルを生成しなくなります。

### EJSを使う場合

`_dev/ejs`配下で記述したEJSコードをHTMLにコンパイルします。ファイル名の先頭に`_`をつけた場合はモジュールファイルとして解釈されるため、直接HTMLファイルを生成しなくなります。

※EJSは**Prettierによるファイルフォーマットに対応していません。**
...
- ウィザードでpackage.json自体を書き換える仕様にすれば余計なパッケージを導入せずにすみそう。必要に応じてStorybook入れるとかそういう設定もできる
