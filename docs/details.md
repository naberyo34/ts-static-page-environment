...
TSのコンパイルについては`ts-loader`ではなく`babel-loader(@babel/preset-typescript)`を利用しています。`@babel/preset-env`のポリフィル挿入機能をTSコンパイル時にも動作させることを期待してこの方針にしているのですが、現状あまり動作検証ができていないため問題点ありましたらご連絡ください。
